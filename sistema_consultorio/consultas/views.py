...
def consultas(request, id):
    ...
    if request.method == 'GET':
        ...
        return render(
            request,
            'consultas.html',
            {'paciente': paciente, 'gravacoes': gravacoes, 'datas': datas, 'humores': humores},
        )
...
def ver_referencias(request, id):
    ...
    gravacoes = Gravacoes.objects.filter(datatreinamento__in=data_treinamento)
...
def send_message(request, id):
    ...
    for g in gravacao.resumo:
        SendMessage().send_message('arcane2', {"number": gravacao.paciente.telefone, "textMessage": {"text": g}})
