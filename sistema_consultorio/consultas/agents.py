...
class BaseAgent:
    llm = ChatOpenAI(model_name='gpt-4.1-mini', temperature=0.7, openai_api_key=settings.OPENAI_API_KEY)
    language: str = 'pt-br'
    audience: str = 'Psicologos e pacientes'
...
class Summaries(BaseModel):
    summaries: list[str] = Field(description='A lista de resumos')
...
class RAGContext:
    ...
    def retrieval(self, id_pergunta, id_paciente, k=5):
        ...
        db_path = f'{self.db_path}_{id_paciente}'
        vectordb = FAISS.load_local(db_path, self.embeddings, allow_dangerous_deserialization=True)
    ...
    def _extract_date_from_question(self, pergunta):
        # Qual a principal queixa na consulta do dia 05/00/0000
        # 00/00/0000
        # 0000-00-00
